...
2. 에지 네트워크 : 캐시 호스트가 존재하는곳의 IP 주소
   ...
   * 이후 AWS 글로벌 네트워크를 통해 선택된 리전(원본 Origin) 엔드포인트(클라이언트 입장에서 보이는 URL 진입점)로 전달한다.
...
* CloudFront에 배포하면 고유 도메인 (`dxxxx.cloudfront.net`)이 생성함과 동시에 호스팅 된다
   * 예를 들어 : **S3 정적 웹 호스팅 엔드포인트(클라이언트 입장에서 보이는 URL 진입점)** : 정적 웹 사이트를 호스팅하는 Amazon S3 버킷의 접근 주소

---

> ### 4. Global Accelerator

#### 엣지 Anycast 진입 + AWS 백본 경유 + 건강한 리전(원본 Origin) 엔드포인트(클라이언트 입장에서 보이는 URL 진입점) 선택을 제공하는 L4 가속기.

```
1. "플레이어" → 
2. "가까운 AWS 엣지 POP Anycast 진입" → 
3. "최적의 리전(원본 Origin) 엔드포인트(클라이언트 입장에서 보이는 URL 진입점)(ALB/NLB/EC2/EIP)"까지 TCP/UDP 게임 트래픽 자체 가속 운반
```

#### 2). 클라이언트는 GA의 Anycast IP와 세션을 맺고, 통신은 선택된 엔드포인트(클라이언트 입장에서 보이는 URL 진입점)로만 통신한다.
...
1. CloudFront : S3/ALB/API Gateway 등 HTTP/HTTPS 기반 컨텐츠를 전 세계 엣지에서 캐시·전달하는 L7 CDN
   ...
   1. CloudFront가 안붙었을떄.
    ...
    2. CloudFront가 붙었을때.
        ...
        모든 트래픽이 곧바로 리전 엔드포인트(클라이언트 입장에서 보이는 URL 진입점)로 꽂힌다
   ```
2. GA : 멀티리전 엔드포인트(클라이언트 입장에서 보이는 URL 진입점) TCP/UDP 트래픽을 가속하는 L4 가속기
