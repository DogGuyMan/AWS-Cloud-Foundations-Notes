...
4번
정답: A. 리전 내 하나 이상의 데이터 센터로 구성된, 서로 격리된 인프라 단위
보기 해설
     A. (O)
     AZ = 하나 이상의 물리 데이터 센터. 리전 내에서 서로 독립된 전력/네트워크/물리 인프라.

     B. 전 세계 모든 데이터 센터 그룹 (X)
     이건 “글로벌 인프라” 전체 수준.
...
11번
정답: B. 사용자 위치에 따라 지연이 가장 낮은 리전에 트래픽을 보내기 위해

보기 해설
     A. 특정 엔드포인트로만 트래픽 (X)
     이건 단순 A 레코드/단일 엔드포인트 설정.

     B. 가장 낮은 네트워크 지연 리전으로 트래픽 (O)
     Latency-based routing의 목적.

     C. 모든 리전에 동일 비율 분산 (X)
     Weighted routing.

     D. 장애가 있는 리전에만 트래픽 (X)
     말도 안 됨. 장애가 있으면 반대로 보내지 않아야 한다.
...
정답: A, B
     ...
     보기 해설
     A. 여러 리전/AZ 통한 HA/FT (O)
     글로벌 인프라의 핵심 목적 중 하나.

     B. 엣지/POP를 통한 저지연 (O)
     CloudFront/Global Accelerator 등으로 전 세계 사용자에게 빠른 응답 제공.

     C. 모든 리전에서 동일 서비스·동일 가격 (X)
     서비스 제공 여부/가격은 리전마다 다를 수 있음.

     D. 모든 국가에 리전 1개씩 (X)
     리전은 전 세계에 고르게 분포되어 있지만, 국가당 1개씩 있는 구조는 아니다.
```
