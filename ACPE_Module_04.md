...
1. **VPC**
   * 논리적으로 격리된 가상 네트워크 
   * 어떤 리소스를 둘지 + 누가 접근할지 둘 다 제어하는 가상 프라이빗 네트워크
   * IP 대역, 서브넷, 라우팅 테이블, 인터넷 게이트웨이, NAT, 보안 그룹 등 설정 
2. **VPN**
   * 온프레미스 ↔ AWS 사이를 암호화된 터널로 연결
   * 네트워크 자체를 제공하는 것이 아니라, 기존 둘을 연결
...
1. **Cloud**는 **리소스를 배치하는 공간**이다. 단지..
    ...
    * **VPC** : **"어떤 리소스를 둘지** + **누가 접근할지"** 둘 다 제어하는 **가상 프라이빗 네트워크를 생성(create a virtual private network)**
        * AWS 내애세도 격리되어 논리적으로 분할된 **IP 네트워크**
          **서로 다른 특정 서브넷에 리소스를 배치하고 AWS 리소스의 프라이빗 IP 범위를 정의할 수 있음**
        * **VPC 가 만들어지면 생기는 일**
          * 기본적으로 **라우팅 테이블이 생성**됩니다. 
            Create Main Route Table
        * **VPC 생성과 독립적인 작업**
          * **서브넷과 인터넷 게이트웨이는 수동**으로 생성해야 한다.
...
2. **서브넷의 글로벌 단위** : **VPC는 여러 AZ를 포괄** 리전 단위로 1대 1 리소스가 아님. 
    * 즉. 여러 서브넷이 있지만서도, 
     서브넷은 반드시 **단일 AZ = 1 : 1 대응이다.** 속함.
    * 라우트 테이블/ACL은 **서브넷 단위**로 적용.

3. **주소 범위 변경 가능성**
   * VPC 생성 시 CIDR 블록을 지정하고 
   * 이후 **임의로 바꾸는 것은 불가능**
...
1. **Public Subnet** *With Internet Gateway*
   ...
   * 공개 리소스 공유에 사용할 수도 있고,
   * 인터넷에서 접근 가능한 웹 서버,정적 파일 등 "공개용 리소스"를 두기 위한 영역
   * 인터넷 액세스가 필요하지만, 다른 비공개 리소스와는 논리적으로 분리해 두기 위한 용도로 사용됨

2. **Private Subnet** *With Private Gateway + VPN Connection*
   * 리소스를 격리하고 비공개할때도 사용가능
    외부에서 직접 접속할 수 없고
    주로 애플리케이션/DB/내부 시스템 등 민감한 데이터를 처리하는 용도로 사용됨
...
#### VPN : 사설망으로 "연결(use to connect)"해주는 Virtual Private Gateway
...
* 온프레미스 기존 네트워크에서 전용 사설망(Virtual Private Cloud(VPC))으로 **연결**
