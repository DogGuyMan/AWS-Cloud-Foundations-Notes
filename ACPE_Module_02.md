...
* **AMI(Amazon Machine Images)**
  ...
  * AMI는 모든 새로운 인스턴스에 대해 일관된 환경을 제공 반복성을 제공한다

* 자동으로 포함되는것
  1. 인스턴스의 루트 볼륨을 위한 템플릿
  2. AMI를 사용하여 인스턴스를 시작할 수 있는 AWS 계정을 제어하는 시작 권한
   *의외로 Permission은 필요 구성은 아니다.*
  3. 인스턴스 시작 시 연결할 볼륨을 지정하는 블록 장치 매핑
...
* 실행 중인 Amazon EC2 인스턴스의 최소 수를 강제 적용
...
* 로드 밸런서가 들어오는 트래픽을 수신하도록 구성하려면 하나 이상의 리스너(Listener)를 지정해야 합니다
...
2. AWS 서비스, 모바일 애플리케이션 또는 HTTP 엔드포인트(클라이언트 입장에서 보이는 URL 진입점)와 같은 이벤트 소스에서 트리거 되도록 코드를 설정합니다.
...
* Docker는 애플리케이션을 테스트, 배포할 수 있는 소프트웨어 플랫폼
...
##### 두 서비스 모두 트래픽 수요에 따라 컨테이너를 자동으로 확장하고, 상태 점검을 처리하며, 라이프사이클을 관리할 수 있다
